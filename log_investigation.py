...
def generate_invalid_user_report():
    ...
    invalid_login_data = log_analysis_lib.filter_log_by_regex(log_path, r'^(.+ \d+) (.{8}).*(\w+).*SRC=(.*?) ')[1]

    # Generate the CSV report
    df = pd.DataFrame(invalid_login_data)
    invalid_filename = f"invalid_users.csv"
    invalid_headings = ('Date', 'Time', 'Username', 'IP Address')
    df.to_csv(invalid_filename, index=False, header=invalid_headings)
...
def generate_source_ip_log(ip_address):
    ...
    SRC_Ip_add = log_analysis_lib.filter_log_by_regex(log_path, r'^(.+ \d+) (.{8}).(\w+).*SRC=(.*?) DPT=(.*?) LEN=(.*?) ')[1]

    # Save all records to a plain text .log file
    df = pd.DataFrame(SRC_Ip_add)
    replace_sub = re.sub(r'\.','_', ip_address)
    text_file = f"source_ip_{replace_sub}.log"
    df.to_csv(text_file, index=False, header=False)
